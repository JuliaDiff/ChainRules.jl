@testset "Maps and Reductions" begin
    ...
    @testset "Map" begin
        ...
        function _test_mixed_map(f, xs)

            primal = map(f, xs)

            # Test there is an `Xrule` for f(x_i)
            # This isn't really a test, it's just a check that we 
            # have something to do the AD with. 
            ∂self = NoTangent()
            @assert ChainRules.rrule(f, first(xs)) != nothing
            @assert ChainRules.frule((∂self, one(first(xs))), f, first(xs)) != nothing

            y1, pb1 = ChainRules.rrule(NoForwardModeAD(), map, f, xs)
            y2, pb2 = ChainRules.rrule(MixedModeAD(), map, f, xs)

            # Test that the tangents are the same
            r = rand_tangent(primal);
            @test all(pb1(r)[3] .≈ pb2(r)[3])

            @testset "$config" for config in (NoForwardModeAD(), MixedModeAD())
                test_rrule(config, map, f, xs; check_inferred = false)
            end
        end
        _psd() = (r = rand(20,20); return r' * r)

        @testset "f: ℛ -> ℛ" begin
            @testset "$f" for f in (sinpi, cospi)
                f = sinpi
                xs = collect(range(0, stop = 20, length = 101))
                _test_mixed_map(f, xs)
            end
        end

        @testset "f: ℛ² -> ℛ²" begin
            @testset "$f" for f in (inv,)
                Random.seed!(1)
                xs = collect(_psd() for _ in 1:10)
                _test_mixed_map(f, xs)
            end
        end

        @testset "f: ℛ² -> ℛ" begin
            @testset "$f" for f in (det,tr)
                Random.seed!(1)
                xs = collect(_psd() for _ in 1:10)
                _test_mixed_map(f, xs)
            end
