...
function rrule(
    ...
    function lu_pullback(ΔF::Composite)
        Δfactors = ΔF.factors
        Δfactors isa AbstractZero && return (NoTangent(), Δfactors, DoesNotExist())
        factors = F.factors
        ...
        return NoTangent(), ∂A, DoesNotExist()
    end
...
function rrule(::typeof(getproperty), F::TF, x::Symbol) where {T,TF<:LU{T,<:StridedMatrix{T}}}
    function getproperty_LU_pullback(ΔY)
        ...
        elseif x === :factors
            ...
            return (NoTangent(), DoesNotExist(), DoesNotExist())
        end
        ∂F = Composite{TF}(; factors=∂factors)
        return NoTangent(), ∂F, DoesNotExist()
    end
...
function rrule(::typeof(cholesky), A::Real, uplo::Symbol=:U)
    ...
    function cholesky_pullback(ΔC::Composite)
        return NoTangent(), ΔC.factors[1, 1] / (2 * C.U[1, 1]), DoesNotExist()
    end
...
function rrule(::typeof(cholesky), A::Diagonal{<:Real}, ::Val{false}; check::Bool=true)
    ...
    function cholesky_pullback(ΔC::Composite)
        ...
        return NoTangent(), Ā, DoesNotExist()
    end
    return C, cholesky_pullback
end
...
function rrule(
    ...
    function cholesky_pullback(ΔC::Composite)
        ...
        return NoTangent(), _symhermtype(A)(Ā), DoesNotExist()
    end
    return C, cholesky_pullback
end
...
function rrule(
    ...
    function cholesky_pullback(ΔC::Composite)
        ...
        return (NoTangent(), UpperTriangular(Ā), DoesNotExist())
    end
    return C, cholesky_pullback
end
